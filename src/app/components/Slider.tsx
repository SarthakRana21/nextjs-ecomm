            ...
            >
                {slides.map(slide => (
                    <div className={`${slide.bg} w-screen h-full flex flex-col gap-16 xl:flex-row`} key={slide.id}>
                        {/* text container */}
                        <div className="h-1/2 xl:h-full xl:w-1/2 flex items-center justify-center gap-8 flex-col 2xl:gap-12">
            ...
            <div className="absolute m-auto left-1/2 bottom-8 flex gap-4">
              {
                slides.map( (slide, index) => (
                  <div className={`w-3 h-3 rounded-full ring-1 ring-gray-600 cursor-pointer flex items-center justify-center ${current === index ? "scale-150 ring-black" : ""}`} key={slide.id} onClick={() => setCurrent(index)}>
                    {current === index && (
